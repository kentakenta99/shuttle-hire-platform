...
**ステータス：** 本番稼働中（Beta）  
**バージョン：** 3.2  
**ドキュメント責任者：** ENG（開発エンジニア）  
**プロダクトオーナー：** 八木健太  
**最終更新：** 2026年6月23日  
**変更履歴：** [`docs/spec/changelog.md`](changelog.md)

> **v3.1 変更内容（2026-06-22）：**
> - **OTPメール認証キャンセル**：ゲストキャンセルにメール認証（6桁OTP）を必須化。第三者不正キャンセルを設計レベルで排除
> - **確認コード暗号乱数化**：連番式（9,999通り）から暗号乱数式（32¹⁰ ≈ 1兆通り）に刷新。総当たり攻撃を封鎖
> - **OTP SHA-256ハッシュ保存**：DBに平文OTPを保存しない。`sha256(otp + "|" + booking_reference)` で保存
> - **OTP試行回数制限**：5回失敗で自動無効化（`attempt_count`カラム追加）
> - **Email-Firstパターン**：メール送信成功後にのみDBにOTPを記録。サイレント失敗を防止
> - **`cancel_otps`テーブル追加**：有効期限10分・pg_cronで24時間後に自動削除・service_role_only RLS
> - **セキュリティドキュメント整備**：データ保持ポリシー・インシデント対応手順・ホテルスタッフ運用ガイドを追加
>
> **v3.0 変更内容（2026-06-22）：**
> - **ゲストセルフキャンセル**：ゲスト確認ページからセルフキャンセル可能。`cancellation_fee` カラム追加
> - **キャンセルポリシー設定**：`cancellation_policies` テーブル追加。ホテル別＋グローバルデフォルトの二層構造
> - **AI設定アシスタント（ALPO）**：Claude API 連携。自然言語でキャンセルポリシーを変更
> - **3時間未満出発警告**：OAGデータと現在時刻を比較し、3時間を切ったら黄色バナー＋カウントダウン表示
> - **Lazy Flightサーチ**：フライト番号不明ゲスト向けにOAG連携サーチ機能追加（全エアライン対応）
> - **MKドアロゴ表示**：ゲストチケットの車両情報欄にMKマーク画像を追加
> - **フロント問い合わせCTA統一**：電話番号削除。「フロントまでお問い合わせください」に統一
> - **キャンセル確認メール**：`sendGuestCancellationEmail()` でキャンセル時に通知メールを送信
>
...
| スコープ内 | スコープ外 |
|---|---|
| ゲスト本人による予約申請（`/request/[slug]`経由・ホテル承認あり） | 決済処理（後請求・月次精算のみ） |
| ホテルスタッフによる代理予約 | GPS追跡・リアルタイム位置情報（Phase 3以降） |
| TMK管理者による枠・予約管理 | 往復予約（片道のみ） |
| ドライバーへの当日乗車リスト配信 | コンシェルジュモニタープログラム（Phase 2以降） |
| ゲストによる予約確認・セルフキャンセル（QRチケット経由） | |
| OAGリアルタイム便情報連携（フライト検索・ターミナル推奨・出発警告） | |
| キャンセルポリシー設定（ホテル別・AI設定アシスタント） | |
...
ゲスト（宿泊客）はアカウント不要でシステムに直接アクセスできる。アクセス経路は2つ。

| 経路 | URL | 認証 | できること |
|---|---|---|---|
| 予約申請 | `/request/[hotel-slug]` | 不要（公開ページ） | フライト番号入力・人数・日付を選んで予約申請。ホテル側の承認で確定 |
| QRチケット確認 | `/confirm/[bookingReference]` | 不要（公開ページ） | 予約内容確認・搭乗QR表示・セルフキャンセル・出発警告の確認 |

ホテルスタッフによる代理予約も引き続き可能。ゲスト申請はホテル側の承認フローを経て確定する。
...
- **確認番号の発行**：予約成功時に暗号乱数形式の確認番号を自動生成（例：`TMK-A3KF9XB2MQ`）。PostgreSQL `gen_random_bytes()` による32¹⁰通りの乱数コード。旧来の連番式（`TMK-YYYYMM-XXXX`、最大9,999通り）は廃止（v3.1）
...
**QRコードの内容：**
- QRコードは `/confirm/[booking_reference]` の公開URLにリンク
...
#### D'. ゲストセルフキャンセル OTP認証フロー（v3.1追加）

ゲストが `/confirm/[bookingReference]` からキャンセルする場合、**OTPメール認証**が必須。

**4ステート遷移：**
```
idle → confirm → otp → done
```

| ステート | 表示内容 | ユーザー操作 |
|---|---|---|
| `idle` | キャンセルボタン | ボタンタップ |
| `confirm` | キャンセル料・注意事項・メール送信の確認 | 「認証コードを送信」ボタン |
| `otp` | マスクメール・6桁入力欄・試行残回数・再送信ボタン | OTP入力 → 確定 |
| `done` | キャンセル完了メッセージ | — |

**OTP発行フロー（Email-Firstパターン）：**
1. 確認番号から予約・ゲストメールを取得
2. レートリミットチェック（1時間3回/確認コード）
3. `crypto.randomInt(100000, 999999)` で6桁OTP生成
4. **Resendでメール送信（先行）** — 失敗したらここで終了
5. メール成功確認後のみ `cancel_otps` テーブルに `sha256(otp + "|" + booking_reference)` をINSERT

**OTP検証フロー：**
1. アクティブなOTPレコードを取得（`expires_at > now()` かつ `used_at IS NULL`）
2. 入力ハッシュと照合
3. 不一致: `attempt_count + 1`。5回到達時に `expires_at = now()` で即時無効化
4. 一致: `used_at = now()` を記録 → `guestCancelBooking()` 実行

**ゲストメール未登録の場合：**
- OTP送信ボタン非表示
- 「フロントデスクにご連絡ください」メッセージを表示
- ホテルスタッフが管理UIからキャンセル対応
...
3. スキャン成功 → `service_orders.status = 'completed'` に更新
...
- **OTPメール認証キャンセル（v3.1追加）**：ゲストキャンセルに6桁OTPメール認証を必須化。確認番号を知るだけではキャンセル不可。第三者不正キャンセルを設計レベルで排除
- **OTP SHA-256ハッシュ保存（v3.1追加）**：DBには `sha256(otp + "|" + booking_reference)` のみ保存。DB侵害時も平文OTPは読み取り不可
- **OTP試行回数制限（v3.1追加）**：5回不一致で自動無効化（`attempt_count`）。OTP総当たりを封鎖
- **確認番号暗号乱数化（v3.1追加）**：`gen_random_bytes(10)` による32¹⁰ ≈ 1兆通りの乱数コード。旧連番式（9,999通り）から全面刷新
- **OTP送信レートリミット（v3.1追加）**：1確認コードあたり1時間3回まで。スパムOTP送信を防止
- **`cancel_otps` 自動削除（v3.1追加）**：`pg_cron` により有効期限から24時間後に自動削除。個人情報最小保持
- **Email-Firstパターン（v3.1追加）**：DB書き込みはメール送信成功後のみ実行。サイレント失敗によるレートリミット消費を防止
...
-- 予約受注（DOS命名規約 v3.2: bookings → service_orders）
CREATE TABLE service_orders (
  id                uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  booking_reference text UNIQUE NOT NULL,      -- TMK-XXXXXXXXXX（暗号乱数10文字）
  ...
  -- DOS integration fields（v3.2追加）
  external_refs     jsonb,                     -- DOS外部参照（予約IDなど）
  dos_sync_status   text NOT NULL DEFAULT 'not_applicable',  -- 'not_applicable'|'pending'|'synced'|'sync_failed'
  booking_source    text NOT NULL DEFAULT 'hotel_staff',     -- 'hotel_staff'|'guest_request'|'api_agent'
  CONSTRAINT valid_booking_status CHECK (status IN ('confirmed','cancelled','completed')),
  CONSTRAINT valid_dos_sync_status CHECK (dos_sync_status IN ('not_applicable','pending','synced','sync_failed')),
  CONSTRAINT valid_booking_source CHECK (booking_source IN ('hotel_staff','guest_request','api_agent'))
);
CREATE INDEX ON service_orders(slot_id, status);
CREATE INDEX ON service_orders(hotel_id, created_at DESC);
...
-- ゲストキャンセル用OTP（v3.1追加）
-- OTPは平文保存しない。sha256(otp + "|" + booking_reference) のハッシュのみ保存
CREATE TABLE cancel_otps (
  id                uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  booking_reference text NOT NULL,      -- v3.2: confirmation_code → booking_reference
  otp_code          text NOT NULL,      -- SHA-256ハッシュ（平文OTPは保存しない）
  attempt_count     int NOT NULL DEFAULT 0,  -- 5回到達で自動無効化
  expires_at        timestamptz NOT NULL DEFAULT now() + interval '10 minutes',
  used_at           timestamptz,        -- NULL = 未使用 / non-NULL = 使用済み
  created_at        timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX idx_cancel_otps_booking_ref ON cancel_otps (booking_reference, expires_at DESC);
ALTER TABLE cancel_otps ENABLE ROW LEVEL SECURITY;
-- service_role_only: 認証ユーザー・未認証ユーザーともにRLS直接アクセス不可
-- Server Action（adminClient）経由のみアクセス許可
CREATE POLICY service_role_only ON cancel_otps FOR ALL USING (false);
-- pg_cron: 毎日午前3時に有効期限から24時間以上経過したレコードを自動削除
...
### 6-3. 確認番号生成ロジック（v3.1更新：暗号乱数化）

> **v3.1変更**：旧実装（連番式 `TMK-YYYYMM-XXXX`、最大9,999通り）を廃止。`gen_random_bytes()` による暗号乱数に全面刷新。

```sql
-- 形式：TMK-XXXXXXXXXX（10文字暗号乱数）
-- 例：TMK-A3KF9XB2MQ
-- 文字セット：アンビギュアスな文字（0/O, 1/I/L）を除外した32文字
-- 組み合わせ数：32^10 ≈ 1兆通り（旧連番の1億倍）
...
| | 旧実装（v3.0以前） | 新実装（v3.1） |
|---|---|---|
| 形式 | `TMK-202606-0042` | `TMK-A3KF9XB2MQ` |
| 組み合わせ数 | 9,999通り | 32¹⁰ ≈ 1兆通り |
| 生成方式 | `nextval()` 連番 | `gen_random_bytes()` 暗号乱数 |
| 総当たり耐性 | ❌ 列挙可能 | ✅ 計算上不可能 |
...
- **driver**：担当便の shuttle_slots と service_orders のみ SELECT 可
- **未認証**：全テーブルへのアクセス拒否
...
ALTER TABLE cancel_otps ENABLE ROW LEVEL SECURITY; -- v3.1追加
...
CREATE POLICY hotel_staff_own_service_orders ON service_orders
  FOR ALL TO authenticated
  USING (
...
-- cancel_otps: 全ユーザー（認証・未認証）からの直接アクセスを拒否（v3.1追加）
-- Server Action の createAdminClient()（サービスロール）経由のみ操作可
-- 呼び出し元 Server Action でロール検証を実施すること
CREATE POLICY service_role_only ON cancel_otps FOR ALL USING (false);
```
...
| H-04P | `/confirm/[bookingReference]` | ゲスト確認ページ（公開） | 認証不要・モバイル最適化・乗車案内表示 |
...
| KPI | 算出式 |
|---|---|
| 今月の予約数 | 当月の `service_orders` レコード数（`status != 'cancelled'`） |
...
**ホテル別予約ランキング：**
- 当月の `service_orders.hotel_id` 別件数を集計・降順ソート
- バーグラフで比率を視覚化
...
## 17. キャンセルポリシー仕様（v3.0追加）

### 17-1. 概要

ゲストがセルフでキャンセルできる機能。ポリシー（締切・違約金率）はホテル別に設定でき、スーパー管理者UIまたはAI設定アシスタントで即日変更可能。

### 17-2. DBスキーマ

```sql
-- 20260622000002_add_cancellation_policies.sql
CREATE TABLE cancellation_policies (
  id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  hotel_id         uuid REFERENCES hotels(id),   -- NULL = グローバルデフォルト
  threshold_hours  numeric(4,1) NOT NULL DEFAULT 2,
  fee_pct          int NOT NULL DEFAULT 25 CHECK (fee_pct BETWEEN 0 AND 100),
  note             text,
  updated_at       timestamptz NOT NULL DEFAULT now(),
...
-- bookings へのキャンセル料カラム追加
ALTER TABLE service_orders ADD COLUMN IF NOT EXISTS cancellation_fee int;
```

### 17-3. ポリシー取得ロジック

```typescript
// ホテル別ポリシーを優先し、なければグローバルデフォルトを使用
...
```typescript
// 出発 threshold_hours 時間前を過ぎていたら fee_pct % の違約金
const departureAt = new Date(`${slot.date}T${slot.departure_time}+09:00`)
const hoursUntil = (departureAt.getTime() - Date.now()) / (1000 * 60 * 60)
const hasFee = hoursUntil < thresholdHours
const cancellationFee = hasFee ? Math.round(totalPrice * feePct / 100) : 0
```
...
| 画面ID | パス | 内容 |
|---|---|---|
| SA-03 | `/admin/superadmin/settings` | キャンセルポリシー設定（フォーム＋AIアシスタントタブ） |
| SA-04 | `/admin/superadmin/settings/agent` | AI設定アシスタント専用ページ |

### 17-6. AI設定アシスタント（ALPO）

Claude API を使用した自然言語設定変更機能。

**インテント種別：**

| インテント | 内容 |
|---|---|
| `update_policy` | ポリシー変更実行（target + changes を含む） |
| `clarify` | 情報不足で確認が必要 |
| `reject` | 対象外リクエスト |

**環境変数：** `ANTHROPIC_API_KEY`（Server Action 内のみ使用）

---

## 18. フライトサーチ（Lazy Search）仕様（v3.0追加）

### 18-1. 概要

フライト番号不明のゲスト向けに、エアライン名＋時間帯 or 目的地で成田出発便を検索できる補助機能。`FlightNumberInput` コンポーネント内に実装。

### 18-2. APIエンドポイント

```
GET /api/search-flights?airline={IATA}&date={YYYY-MM-DD}&timeRange={morning|afternoon|evening}
GET /api/search-flights?airline={IATA}&date={YYYY-MM-DD}&dest={IATA or city name}
```
...
```typescript
type SearchResult = {
  suggestions: FlightSuggestion[]  // 最大20件
}
type FlightSuggestion = {
  iata: string         // 例: NH832
  airline: string      // 例: All Nippon Airways
  terminal: string | null  // 例: 2
  dest: string         // 例: Los Angeles International
  scheduledDep: string | null  // ISO8601
}
```
...
入力が航空会社名（ANA・United等）の場合、IATAコードに変換してから検索。
...
## 19. 3時間未満出発警告仕様（v3.0追加）

### 19-1. 概要

ゲスト確認ページ（`/confirm/[bookingReference]`）でOAGデータの出発時刻と現在時刻（JST）を比較。3時間を切ったら黄色バナーを自動表示。

### 19-2. 実装

- **コンポーネント**: `app/confirm/[bookingReference]/TimeUntilDepartureWarning.tsx`
- **Props**: `date: string`、`departureTime: string`
- **更新間隔**: 1分ごと（setInterval）
- **表示条件**: `hoursUntil < 3`（JST基準）
- **非表示条件**: `hoursUntil >= 3` または乗車確認済み（`status === 'completed'`）

### 19-3. 表示内容

```
⚠️ 時間がタイトです
このシャトル便だと出発が早く、乗り遅れるリスクがあります。
別の便をお選びいただくか、フロントまでお問い合わせください。
出発まで残り X時間 Y分
```
