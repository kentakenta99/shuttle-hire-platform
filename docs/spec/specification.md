...
> - **OTP SHA-256ハッシュ保存**：DBに平文OTPを保存しない。`sha256(otp + "|" + confirmationCode)` で保存
...
| QRチケット確認 | `/confirm/[confirmationCode]` | 不要（公開ページ） | 予約内容確認・搭乗QR表示・セルフキャンセル・出発警告の確認 |
...
ゲストが `/confirm/[confirmationCode]` からキャンセルする場合、**OTPメール認証**が必須。
...
5. メール成功確認後のみ `cancel_otps` テーブルに `sha256(otp + "|" + confirmationCode)` をINSERT
...
- **OTP SHA-256ハッシュ保存（v3.1追加）**：DBには `sha256(otp + "|" + confirmationCode)` のみ保存。DB侵害時も平文OTPは読み取り不可
...
-- ゲストキャンセル用OTP（v3.1追加）
-- OTPは平文保存しない。sha256(otp + "|" + confirmationCode) のハッシュのみ保存
...
ALTER TABLE bookings ADD COLUMN IF NOT EXISTS cancellation_fee int;
```
...
ゲスト確認ページ（`/confirm/[confirmationCode]`）でOAGデータの出発時刻と現在時刻（JST）を比較。3時間を切ったら黄色バナーを自動表示。

### 19-2. 実装

- **コンポーネント**: `app/confirm/[confirmationCode]/TimeUntilDepartureWarning.tsx`
