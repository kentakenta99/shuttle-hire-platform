...
> - **ビジネス目的の明確化**：エミレーツ成田UPの空車問題解消が主目的。ホテルシャトル利用客 = エミレーツUP搭乗客という構造を明記
...
#### ビジネスコンテキスト：エミレーツ成田便「UP空車問題」の解消

本システムの**最重要目的**は、エミレーツ航空成田便における**UP方向（都内→成田）の空車問題解消**である。

- **DOWN**（成田→都内）：エミレーツ乗客を乗せた実車。収益あり ✅
- **UP**（都内→成田）：エミレーツ乗客を迎えに行く空車回送。コストのみ ❌

シャトルハイヤーはこのUP空車にホテル宿泊客（成田行き）を相乗りさせることで、**空車コストを収益に転換する施策**である。ホテルのシャトル利用客 = エミレーツUPに同乗する客、という構造が本システムの本質。

**スロット運用方針：**
- **デフォルト台数**：3台/日を基本とし、前日17時に増減を確定
- **台数根拠**：前日17時時点でエミレーツのUP便数がほぼ確定 → Booknetics（Phase 4）またはTMK手動で調整
- **スロット公開タイミング**：前日17時以降にスロットを `open` にする（それ以前は非公開）
- **出発枠**：11:00〜15:00の範囲内で等間隔設定（エミレーツ便と連動）
