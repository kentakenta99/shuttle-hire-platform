...
#### ビジネスコンテキスト：エミレーツ片送り解消施策

本システムの直接的な背景は**エミレーツ航空の成田便における「片送り問題」**。TMKはエミレーツ乗客を成田まで送るが、その帰り便（成田→都内）が空車になる。シャトルハイヤーはこの空車回送コストを埋める収益化施策であり、成田行き台数は当日午前8時のエミレーツ便情報が確定した時点でほぼ確定する。

- **出発枠**：11時〜15時の範囲内で設定（エミレーツ便と連動）
- **使用車両**：Vクラスベンツ / アルファード（共通）
- **台数確定**：当日8時にエミレーツ片道台数確定 → それまでは仮台数（3〜5台）で枠公開
...
空きがあれば即予約 → 確認票発行
    ↓
締切済みの場合：「満席」または「受付終了」を表示 → 代替案を案内
```
...
### 4-3. ドライバーUI（スマホファースト）

#### 実装方針：2フェーズ展開

| | Phase 1（スタンドアロン） | Phase 2（DRアプリ統合） |
|---|---|---|
| 実装形態 | `/driver/` スタンドアロンWebアプリ | Dispatch OS DRアプリ「今日」タブの「シャトル便」サブメニュー |
...
| D-03 | `/driver/slots/[id]` | 乗車リスト | 乗客一覧・ステータス更新 |
...
| R6 | ドライバー通知タイミング | **前日20時までにアサイン確定**。出発枠の台数は当日8時にエミレーツ片道台数確定後に最終調整 | ✅ 確定 |
| R7 | 満席時のフロー | ホテルUIに「**通常ハイヤーでのご案内が可能です（特別割引）**」のメッセージと配車センター連絡先を表示。システム上での代替予約は不要 | ✅ 確定 |
